...
const TwitterSection = () => {
  ...
  const [TwitterData, setTwitterData] = useState<TwitterData | null>(
    null
  );
  const [profileUrl, setProfileUrl] = useState<string>("");

  const handleDisconnect = () => {
    // Simulate a call to the backend to disconnect the user's Twitter account
    // Set isConnected to false and clear the user's Twitter data from the state
  ...
  const formattedUsername =
    username.length > 18 ? `${username.slice(20, -1)}`  : username; //if the username is greater than 15 characters it slices the url, else it shows the whole name

  const handleConnectToProfile = () => {
    // Simulate a call to the backend to connect the user's Twitter account using the profile URL
    // Set isConnected to true and store the user's Twitter data in the state
    if (profileUrl === "") {
      return alert("Please enter a username"); // do nothing if profileUrl is empty
    }
    setIsConnected(true);
    setTwitterData({
      ...
      {isConnected ? (
        <div className="flex items-center flex-row  justify-start">
          <p className="font-bold mr-2 ">
            {" "}
            <span className="font-normal mr-1">Twitter Profile linked:</span>{" "}
        ...
        <>
          <div className="flex flex-row justify-start sm:justify-evenly">
            <label className="font-[600] text-sm sm:text-lg">
              Enter your Twitter profile or URL:
              <input
            ...
            <div
              className=" cursor-pointer lg:mr-12 twitter mt-[1px] flex justify-center w-7 h-7 tracking-wide font-semibold text-white text-center rounded-xl"
