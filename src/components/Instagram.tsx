...
const InstagramSection = () => {
  ...
  const formattedUsername =
    username.length > 21 ? `${username.slice(26, -1)}` : username; //if the username is greater than 15 characters it slices the url, else it shows the whole name

  const handleConnectToProfile = () => {
    // Simulate a call to the backend to connect the user's Instagram account using the profile URL
      ...
      {isConnected ? (
        <div className="flex items-center flex-row  justify-start">
          <p className="font-bold mr-2 ">
            {" "}
            <span className="font-normal mr-1">Instagram Profile linked:</span>{" "}
        ...
        <>
          <div className="flex flex-row justify-start sm:justify-evenly">
            <label className="font-[600] text-sm sm:text-lg">
              Enter your Instagram profile or URL:
              <input
            ...
            <div
              className=" cursor-pointer lg:mr-16 instagram mt-[1px] flex justify-center w-7 h-7 tracking-wide font-semibold text-white text-center rounded-xl"
